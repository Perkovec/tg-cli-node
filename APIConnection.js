...
class APIConnection extends events.EventEmitter {
	...
	add_contact(phnom,fname,lname) {
		return new Promise((resolve, reject) => {
			this._executeCommand('add_contact',phnom,fname,lname);
			this._cb_receiver = data => {
				if (data.hasOwnProperty("result") | !data ) if(data.result == "FALSE") reject(data) ;
				else resolve(data);
			};
		});
	}

	del_contact(username) {
		return new Promise((resolve, reject) => {
			this._executeCommand('del_contact',username);
			this._cb_receiver = data => {
	...
	contact_search(user) {
		return new Promise((resolve, reject) => {
			this._executeCommand('contact_search',user);
			this._cb_receiver = data => {
				if(data.result == "SUCCESS") resolve(data); 
				else reject(data);
			};
